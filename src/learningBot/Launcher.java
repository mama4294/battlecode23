...
// import learningBot.util.FastMath;
...
public class Launcher extends Unit {
  ...
  static void run() throws GameActionException {
    ...
    if (!Comm.isSymmetryConfirmed) {
      ...
      if (rc.canSenseLocation(symGuessLoc)) {
        ...
        if (
          hq == null || hq.type != RobotType.HEADQUARTERS || hq.team != oppTeam
        ) {
          Comm.eliminateSym(Comm.symmetry);
        }
      }
    }
    if (!MapRecorder.initialized) MapRecorder.initializeMapRecorder();
      ...
      if (robot.team == myTeam) {
        if (robot.type == RobotType.LAUNCHER) {
          ...
          if (
            ...
            (
              robot.getHealth() == groupingTarget.getHealth() &&
              robot.location.distanceSquaredTo(enemyHQLoc) <
              groupingTarget.location.distanceSquaredTo(enemyHQLoc)
            )
          ) {
        ...
        if (robot.type == RobotType.HEADQUARTERS) {
          ...
          if (
            !Comm.isSymmetryConfirmed &&
            getClosestDis(robot.location, Comm.enemyHQLocations) != 0
          ) {
            Comm.eliminateSym(Comm.symmetry);
          }
          ...
          robot.type == RobotType.DESTABILIZER
        ) {
          ...
          if (
            dis <= ATTACK_DIS && (backupTarget == null || dis < backupTargetDis)
          ) {
            backupTarget = robot;
            backupTargetDis = dis;
  ...
  static void macro() throws GameActionException {
    ...
    if (
      closeFriendsSize < 3 && (rc.getRoundNum() - lastLauncherAttackRound) < 10
    ) {
      if (rc.isMovementReady() && groupingTarget != null) {
        ...
        indicator +=
          String.format("cacheGroup%s,", cachedGroupingTarget.location);
        follow(cachedGroupingTarget.location);
        rc.setIndicatorLine(
          rc.getLocation(),
          cachedGroupingTarget.location,
          0,
          255,
          0
        );
  ...
  static boolean tryIslandStuff() throws GameActionException {
    ...
    boolean needHeal =
      (rc.getHealth() < MAX_HEALTH && state == HEALING) ||
      rc.getHealth() < HEALING_CUTOFF;
    if (
      state == HEALING &&
      !needHeal &&
      (ourTeamStrength >= 3 || rc.getRoundNum() - lastLauncherAttackRound > 50)
    ) {
      islandTargetIndex = 0;
    ...
    if (
      state == ATTACKING &&
      needHeal &&
      rc.getRoundNum() - lastFailedHealingTurn > 100
    ) {
    ...
    for (int i = islandIndexes.length; --i >= 0;) {
      ...
      if (islandIndex == islandTargetIndex) {
        ...
        if (
          ...
          (
            occupyingTeam == Team.NEUTRAL ||
            (
              occupyingTeam == myTeam &&
              rc.senseAnchor(islandIndex).totalHealth ==
              Anchor.STANDARD.totalHealth
            )
          )
        ) {
          islandTargetLoc = null;
  ...
  static void micro() throws GameActionException {
    ...
    if (
      attackTarget == null &&
      commTarget != null &&
      !rc.canSenseLocation(commTarget)
    ) {
    ...
    if (
      Comm.getClosestFriendlyIslandIndex() != 0 &&
      rc.getHealth() < HEALING_CUTOFF
    ) {
    ...
    if (rc.isMovementReady() && rc.isActionReady()) {
      if (chaseTarget != null) {
        ...
        cachedRound = rc.getRoundNum();
        if (
          rc.getHealth() > chaseTarget.health ||
          ourTeamStrength > 2 ||
          chaseTarget.type != RobotType.LAUNCHER
        ) {
        ...
        cachedEnemyLocation != null && rc.getRoundNum() - cachedRound <= 2
      ) {
  ...
  private static void chase(MapLocation location) throws GameActionException {
    ...
    Direction[] dirs = {
      forwardDir,
      forwardDir.rotateLeft(),
      forwardDir.rotateRight(),
      forwardDir.rotateLeft().rotateLeft(),
      forwardDir.rotateRight().rotateRight(),
    };
    ...
    for (Direction dir : dirs) {
      if (
        rc.canMove(dir) &&
        rc.getLocation().add(dir).distanceSquaredTo(location) <= ATTACK_DIS
      ) {
        int canSee = 0;
        ...
        for (int i = enemyLauncherCnt; --i >= 0;) {
          int newDis = rc
            .getLocation()
            .add(dir)
            .distanceSquaredTo(enemyLaunchers[i].location);
          if (
            newDis <= 4 ||
            (newDis <= VISION_DIS && !hasCloud && !enemyInCloud[i])
          ) {
            canSee++;
          }
        }
  ...
  private static void kite(MapLocation location) throws GameActionException {
    ...
    Direction[] dirs = {
      ...
      backDir.rotateLeft(),
      backDir.rotateRight(),
      backDir.rotateLeft().rotateLeft(),
      backDir.rotateRight().rotateRight(),
    };
    Direction bestDir = null;
    int minCanSee = Integer.MAX_VALUE;
    boolean bestHasCloud = false;
  ...
  private static final int[] GUESS_ATTACK_DX = {
    -4,
    -3,
    -3,
    -3,
  ...
  private static final int[] GUESS_ATTACK_DY = {
    0,
    -2,
    -1,
    0,
  ...
  private static void tryGuessAttack() throws GameActionException {
    ...
    if (cachedEnemyLocation != null && rc.getRoundNum() - cachedRound <= 8) {
      ...
      for (int i = 9; --i >= 0;) {
        ...
        switch (i) {
          ...
          case 8:
            loc = cachedEnemyLocation;
            break;
          case 7:
            loc = cachedEnemyLocation.add(targetDir);
            break;
          case 6:
            loc = cachedEnemyLocation.add(targetDir.opposite());
            break;
          case 5:
            loc = cachedEnemyLocation.add(targetDir.rotateLeft());
            break;
          case 4:
            loc = cachedEnemyLocation.add(targetDir.rotateRight());
            break;
          case 3:
            loc = cachedEnemyLocation.add(targetDir.opposite().rotateRight());
            break;
          case 2:
            loc = cachedEnemyLocation.add(targetDir.opposite().rotateLeft());
            break;
          case 1:
            loc =
              cachedEnemyLocation.add(targetDir.rotateRight().rotateRight());
            break;
          case 0:
            loc = cachedEnemyLocation.add(targetDir.rotateLeft().rotateLeft());
            break;
        }
        if (!rc.canSenseLocation(loc) && rc.canAttack(loc)) {
          //          char val = MapRecorder.vals[loc.x * mapHeight + loc.y];
          //          if (
          //            (val & MapRecorder.SEEN_BIT) != 0 &&
    ...
    if (rc.senseCloud(rc.getLocation())) { // I am in a cloud, try attack a tile I can't see
      ...
      for (int i = 10; --i >= 0;) {
        ...
        //   curX + GUESS_ATTACK_DX[j],
        //   curY + GUESS_ATTACK_DY[j]
        // );
  ...
  private static RobotInfo getImmediatelyAttackableTarget() {
    ...
    for (int enemy_i = enemyLauncherCnt; --enemy_i >= 0;) {
      ...
      for (int friend_i = friendlyLauncherCnt; --friend_i >= 0;) {
        int friendEnemyDis =
          friendlyLaunchers[friend_i].location.distanceSquaredTo(
              enemy.location
            );
        if (
          friendEnemyDis <= 4 ||
          (
            friendEnemyDis <= ATTACK_DIS &&
            !enemyInCloud[enemy_i] &&
            !friendInCloud[friend_i]
          )
        ) {
          canAttackFriendCnt++;
        }
      ...
      int hitRequired =
        (enemy.getHealth() + canAttackFriendCnt * DAMAGE - 1) /
        DAMAGE /
        canAttackFriendCnt;
