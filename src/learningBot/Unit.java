...
public class Unit extends RobotPlayer {
  ...
  static int randInt(int bound1, int bound2) {
  ...
  static void randomMove() throws GameActionException {
    int starting_i = randInt(0, 256) % Constants.directions.length;
  ...
  static void tryMoveDir(Direction dir) throws GameActionException {
    if (rc.isMovementReady() && dir != Direction.CENTER) {
      if (rc.canMove(dir) && canPass(dir)) {
        ...
        rc.canMove(dir.rotateRight()) && canPass(dir.rotateRight(), dir)
      ) {
        rc.move(dir.rotateRight());
      } else if (
        rc.canMove(dir.rotateLeft()) && canPass(dir.rotateLeft(), dir)
      ) {
...
//  }
  ...
  private static int currentTurnDir = randInt(0, 256) % 2;
  ...
  static void moveToward(MapLocation location) throws GameActionException {
    ...
    if (
      !location.equals(lastPathingTarget) || lastPathingTurn < turnCount - 4
    ) {
      pathingCnt = 0;
      stuckCnt = 0;
    }
      ...
      if (pathingCnt == 0) {
        ...
        if (dirCanPass || dirRightCanPass || dirLeftCanPass) {
          ...
          while (!canPass(dir) && pathingCnt != 8) {
            ...
            prv[pathingCnt] = dir;
            pathingCnt++;
            if (currentTurnDir == 0) dir = dir.rotateLeft(); else dir =
              dir.rotateRight();
          }
          if (pathingCnt == 8) {
        ...
        while (
          ...
          !canPass(
            currentTurnDir == 0
              ? prv[pathingCnt - 1].rotateLeft()
              : prv[pathingCnt - 1].rotateRight()
          )
        ) {
          prv[pathingCnt] =
            currentTurnDir == 0
        ...
        Direction moveDir = pathingCnt == 0
          ? prv[pathingCnt]
          : (
            currentTurnDir == 0
              ? prv[pathingCnt - 1].rotateLeft()
  ...
  static int getSteps(MapLocation a, MapLocation b) {
    ...
    if (xdif < 0) xdif = -xdif;
    if (ydif < 0) ydif = -ydif;
    if (xdif > ydif) return xdif; else return ydif;
  }
  ...
  static int getTurnDir(Direction direction, MapLocation target)
    ...
    if (byteCodeRem < BYTECODE_CUTOFF) return randInt(0, 256) % 2;
    ...
    while (pathingCnt_ > 0) {
      ...
      while (
        pathingCnt_ > 0 &&
        canPass(now.add(prv_[pathingCnt_ - 1]), prv_[pathingCnt_ - 1])
      ) {
        pathingCnt_--;
      }
      if (
        pathingCnt_ > 1 &&
        canPass(now.add(prv_[pathingCnt_ - 1]), prv_[pathingCnt_ - 2])
      ) {
        pathingCnt_ -= 2;
      }
      while (
        pathingCnt_ > 0 &&
        !canPass(
          now.add(prv_[pathingCnt_ - 1].rotateLeft()),
          prv_[pathingCnt_ - 1].rotateLeft()
        )
      ) {
      ...
      Direction moveDir = pathingCnt_ == 0
        ? prv_[pathingCnt_]
        : prv_[pathingCnt_ - 1].rotateLeft();
      now = now.add(moveDir);
    }
    ...
    while (pathingCnt_ > 0) {
      ...
      while (
        pathingCnt_ > 0 &&
        !canPass(
          now.add(prv_[pathingCnt_ - 1].rotateRight()),
          prv_[pathingCnt_ - 1].rotateRight()
        )
      ) {
    ...
    //find best direction
    if (moveLeft == -1 || moveRight == -1) return randInt(0, 256) % 2;
    if (
      moveLeft +
      getSteps(leftend, target) <=
      moveRight +
      getSteps(rightend, target)
    ) return 0; else return 1;
  }

  static boolean canPass(MapLocation loc, Direction targetDir)
    throws GameActionException {
  ...
  static boolean canPass(Direction dir, Direction targetDir)
    throws GameActionException {
    ...
    if (robot == null) return true;
    return randInt(0, 256) % 4 == 0; // Does rng help here? Each rng is 10 bytecode btw
  }
