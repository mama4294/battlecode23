...
public class Comm extends RobotPlayer {
  ...
  //Shared array info
  //1-4: friendly HQ locations (12 bits each)
  ...
  // private static FastIterableIntSet changedIndexes = new FastIterableIntSet(
  //   ARRAY_LENGTH
  // );
  ...
  public static void turn_starts() throws GameActionException {
    ...
    if (
      (turnCount <= 1 && rc.getType() == RobotType.HEADQUARTERS) ||
  ...
  public static int HQInit(MapLocation location, int HQID) {
    ...
    for (int i = 0; i < GameConstants.MAX_STARTING_HEADQUARTERS; i++) {
      ...
      if (friendlyHQLocations[i] == null) {
        // Assign the given location to the current slot in the array.
        friendlyHQLocations[i] = location;

        // Write the bits corresponding to the location in an external storage mechanism.
        // The method loc2int() converts the MapLocation to an integer representation.
        writeBits(i * 12, 12, loc2int(location));

        // Update the variable numHQ to reflect the number of initialized HQs.
        numHQ = i + 1;

        // Perform a guessing operation (not specified in the provided code).
        guessSym();

        // Return the index of the initialized HQ.
        return i;
      }
    }

    // If this point is reached, it indicates a failure in finding an available slot for HQ initialization.
    assert false;
  ...
  private static void updateHQLocations() throws GameActionException {
    ...
    if (
      turnCount == 1 &&
      rc.getType() == RobotType.HEADQUARTERS &&
      !isSymmetryConfirmed
    ) {
      for (int i = numHQ; --i >= 0;) {
        for (int sym = 3; --sym >= 0;) {
          if (isSymEliminated[sym]) continue;
          ...
          if (
            robot == null ||
            robot.type != RobotType.HEADQUARTERS ||
            robot.team != oppTeam
          ) {
  ...
  //       int startingBit =
  //         WELL_INFO_BIT + ((resourceID - 1) * NUM_WELLS + i) * 12;
  ...
  public static void reportWells(int resourceID, MapLocation wellLocation) {
    ...
    int maxDis =
      friendlyHQLocations[closestHQID].distanceSquaredTo(wellLocation);
    ...
    for (int i = 0; i < NUM_WELLS; i++) {
      ...
      int original_dis = Unit.getClosestDis(
        closestWells[resourceID][i],
        friendlyHQLocations
      );
    ...
    if (updateIndex != -1) {
      // update shared array
      int startingBit =
        WELL_INFO_BIT + ((resourceID - 1) * NUM_WELLS + updateIndex) * 12;
  ...
  public static void amplifierUpdate(int index) throws GameActionException {
    writeBits(
      AMPLIFIER_BIT + AMPLIFIER_LEN * index,
      AMPLIFIER_LEN,
      4 + rc.getRoundNum() % 4
    );
  }
    ...
    if (canWrite) {
      ...
      for (int i = Math.min(5, enemies.length); --i >= 0;) {
        ...
        switch (robot.type) {
          ...
          case AMPLIFIER:
            rc.writeSharedArray(
              ENEMY_ARR_IDX + writeLoc,
              loc2int(robot.location) + (curRound << 12)
            );
            writeLoc++;
  ...
  private static int readBits(int left, int length) {
    ...
    while (left <= endingBitIndex) {
      ...
      rv =
        (rv << (right - left + 1)) +
        (
          (buffered_share_array[left / 16] % (1 << (16 - left % 16))) >> (
            15 - right % 16
          )
        );
      left = right + 1;
  ...
  private static void writeBits(int startingBitIndex, int length, int value) {
    ...
    while (current_length > 0) {
      ...
      int original_value =
        (buffered_share_array[left / 16] % (1 << (16 - left % 16))) >> (
          15 - current_ending % 16
        );
      int new_value = value % (1 << len);
      value >>= len;
      if (new_value != original_value) {
        // changedIndexes.add(current_ending / 16);
        buffered_share_array[current_ending / 16] ^=
          (new_value ^ original_value) << (15 - current_ending % 16);
      }
      current_length -= len;
    }
  }
